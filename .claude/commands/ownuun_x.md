...
X(Twitter) 콘텐츠를 **크롤링 → 분석 → 게시**하는 전체 파이프라인입니다.
...
/ownuun_x (독립 파이프라인)
    │
    ├─ Phase 1: 크롤링
    │      └─ node scripts/crawl-x.mjs → DB 저장 (pending_analysis)
    │
    ├─ Phase 2: 에이전트 직접 분석
...
```bash
cd /Users/ownuun/conductor/workspaces/v2-v1/kiev && node scripts/crawl-x.mjs --limit=20
```
...
async function check() {
  ...
  console.log(status + ' X pending_analysis: ' + (count || 0) + '개');
